...
class IJKPlayer : AbsMediaCore() {
    ...
    override fun init() {
        ...
        //设置监听
        player?.setOnPreparedListener { absMediaCoreOnLisenter?.onPrepared(this) }
        player?.setOnCompletionListener { absMediaCoreOnLisenter?.onCompletion(this) }
        player?.setOnBufferingUpdateListener { p0, p1 -> absMediaCoreOnLisenter?.onBufferingUpdate(this, p1) }
        player?.setOnSeekCompleteListener { mp -> absMediaCoreOnLisenter?.onSeekComplete(this) }
        player?.setOnVideoSizeChangedListener { mp, width, height, sar_num, sar_den -> absMediaCoreOnLisenter?.onVideoSizeChanged(this, width, height, sar_num, sar_den) }
        player?.setOnErrorListener { mp, what, extra -> absMediaCoreOnLisenter?.onError(this, what, extra)!! }
        player?.setOnInfoListener { mp, what, extra -> absMediaCoreOnLisenter?.onInfo(this, what, extra)!! }
        player?.setOnTimedTextListener { mp, text -> absMediaCoreOnLisenter?.onTimedText(this) }
    }
    ...
    private fun surfaceCreated() {
        ...
        player!!.setOnPreparedListener {
            super.absMediaCoreOnLisenter?.onPrepared(this)
        }
